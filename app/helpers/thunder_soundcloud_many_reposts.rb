...
module ThunderSoundcloudManyReposts
  ...
  def self.write_to_csv(user)
    ...
    # ThunderSoundcloudManyReposts::format_user(user)
  end
  ...
  def self.attempt_user(id)
    ...
    return unless user['errors'].nil? && ThunderSoundcloudManyReposts.many_followers?(user['followers_count'])
    ...
    # unless tracks.first
    return unless track_reposts.any? && ThunderSoundcloudManyReposts::recent?(track_reposts.first['created_at'])

    ThunderSoundcloudManyReposts::write_to_csv(user)
